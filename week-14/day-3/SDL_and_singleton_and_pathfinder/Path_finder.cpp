...
void Path_finder::route_planning(int start_x, int start_y, int finish_x, int finish_y) {
  /*get the route*/
  string route = pathFind(start_x, start_y, finish_x, finish_y);
  if (route == "") {    
  /*error handling event needed*/
  }
...
void Path_finder::initialize_tile_map() {
...
void Path_finder::clean_tile_map() {
  for (int x = 0; x < MAP_SIZE; x++) {
    for (int y = 0; y < MAP_SIZE; y++) {
      Singleton::getInstance()->tile_map[x][y] = 0;
    }
  }
}

void Path_finder::fill_with_plus_pattern() {
...
void Path_finder::set_coord_on_map(int x, int y) {
